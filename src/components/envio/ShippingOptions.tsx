...
import { Truck, FileText, Upload, AlertCircle, CheckCircle2, ArrowDown } from 'lucide-react';
...
export default function ShippingOptions({
  ...
  // State to track if we're in compact view mode
  ...
  const handleSelectManuableService = (service: ManuableRate) => {
    ...
    // Scroll to the label generator when a service is selected
    setTimeout(() => {
      const labelGenerator = document.getElementById('manuable-label-generator');
      if (labelGenerator) {
        labelGenerator.scrollIntoView({ behavior: 'smooth', block: 'center' });
      }
    }, 100);
  };
  ...
  const handleLabelGenerated = (data: ManuableLabelResponse) => {
    ...
    setLabelData(data);
    // Auto-collapse to compact view after label generation
    setCompactView(true);
  };
    ...
    <div className="space-y-6 p-4">
      ...
      <div className="flex items-center justify-between pb-4 border-b border-gray-200">
        ...
        {(isExternalComplete || isManuableComplete) && (
          ...
          >
            {compactView ? 'Ver detalles' : 'Vista compacta'}
          </button>
        )}
      </div>

      {/* If we have completed one of the options and in compact view, show condensed UI */}
      {(isExternalComplete || isManuableComplete) && compactView ? (
        <div className={`p-5 rounded-xl ${
          isExternalComplete ? 'bg-blue-50 border border-blue-200' : 
          'bg-green-50 border border-green-200'
        }`}>
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              <CheckCircle2 className={`w-5 h-5 ${isExternalComplete ? 'text-blue-600' : 'text-green-600'}`} />
              <div>
                <h4 className="font-medium text-gray-900">
                  {isExternalComplete ? 'Guía Externa Configurada' : 'Etiqueta Manuable Generada'}
                </h4>
                <p className="text-sm text-gray-600">
                  {isExternalComplete 
                    ? `${externalLabelData.carrier} - ${externalLabelData.trackingNumber}`
                    : `${labelData?.tracking_number} - ${selectedManuableService?.carrier}`}
                </p>
              </div>
            </div>
            <button 
              onClick={() => setCompactView(false)}
              className="px-3 py-1 bg-white rounded-md text-sm font-medium shadow-sm hover:bg-gray-50"
            >
              Editar
            </button>
          </div>
          ...
          {selectedOption === 'manuable' && (
            <div className="bg-white border border-gray-200 rounded-xl p-6">
              ...
              {labelData ? (
                ...
                <>
                  {/* Show Manuable Rates Component First */}
                  <div className="mb-6">
                  ...
                  {selectedManuableService && (
                    <div className="mt-8 pt-6 border-t border-gray-200" id="manuable-label-generator">
                      ...
                      <div className="bg-green-50 p-4 rounded-lg border border-green-200">
                        <h5 className="font-medium text-lg text-green-800 mb-3">Generar Etiqueta</h5>
